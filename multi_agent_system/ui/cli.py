...
class AgentCLI:
    ...
    async def run_interactive(self):
        ...
        while True:
            ...
            # /analyze komutu — derinlemesine kod analizi (ZIP / GitHub / klasör)
            if goal.strip().startswith("/analyze"):
                source = goal.strip()[8:].strip().strip('"').strip("'")
                if not source:
                    console.print(
                        "[yellow]Kullanım:[/yellow] /analyze <klasör_yolu | zip_yolu | github_url>\n"
                        "[dim]Örnekler:\n"
                        "  /analyze C:\\Projeler\\eski_proje\n"
                        "  /analyze proje.zip\n"
                        "  /analyze https://github.com/kullanici/repo[/dim]"
                    )
                    continue

                console.print(f"\n[bold cyan]🔍 Analiz başlıyor: {source}[/bold cyan]")
                ...
                task = Task(
                    task_id="analyze_cli",
                    description=f"Bu projeyi analiz et: {source}",
                ...
                if not response.success:
                    console.print(f"[red]❌ Analiz hatası: {response.error}[/red]")
                    continue

                # Raporu yazdır
                report = analyzer.format_report(response.content)
                console.print(report)

                # Orchestrator'a analiz bağlamını yükle
                if self.orchestrator:
                    fix_plan = response.content.get("fix_plan", [])
                    self.orchestrator.shared_context = (
                        f"\nANALİZ SONUCU — {response.content.get('project_name', '')}:\n"
                        f"Amaç: {response.content.get('project_purpose', '')}\n"
                        f"Bug sayısı: {len(response.content.get('bugs_found', []))}\n"
                        f"Fix planı: {len(fix_plan)} adım\n"
                        f"Özet: {response.content.get('summary', '')}\n"
                    )

                console.print(
                    "[bold green]Analiz tamamlandı![/bold green] "
                    "Şimdi ne düzeltmek istiyorsun?\n"
                    "[dim]Örnek: 'security_issues kısmındaki hardcoded şifreyi düzelt'[/dim]\n"
                    "[dim]Örnek: 'Tüm bug'ları düzelt ve testleri yaz'[/dim]\n"
                )
                continue
